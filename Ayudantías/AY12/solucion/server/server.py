...
class Server:
        ...
        while True:
            try:
                ...
                with self.info_lock:
                    ...
                    self.client_events[addr] = threading.Event()
                ...
                if len(self.en_espera) >= 2:
                    self.client_events[self.en_espera[0]].wait()
                    self.client_events[self.en_espera[1]].wait()
    ...
    def procesar_mensaje(self, addr, msg):
        ...
        if accion == "jugada":
            fil, col = msg["fila"], msg["columna"]
            print(fil, col)
            partida = self.obtener_partida(addr)
            if not partida:
                return
            resultado = partida.procesar_jugada(addr, fil, col)
            self.broadcast(partida, resultado)

            if resultado["accion"] in ("ganador", "empate"):
                self.eliminar_partida(partida)
        
        if accion == "setup listo":
            self.client_events[addr].set()
