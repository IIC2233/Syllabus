...
class Inicio(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Inicio de la Casa")

        self.layout = QVBoxLayout()

        # Label que indica que se debe ingresar la contraseña
        self.label_input = QLabel("Ingrese la contraseña de su hogar:", self)
        self.label_input.setStyleSheet("font-size: 28px;")
        self.layout.addWidget(self.label_input)

        # QLineEdit para ingresar la contraseña
        self.input = QLineEdit('', self)
        self.input.setStyleSheet("font-size: 26px;")
        self.layout.addWidget(self.input)

        # Botón para establecer nueva contraseña
        self.boton_contrasena = QPushButton("Establecer Nueva Contraseña")
        self.boton_contrasena.setStyleSheet("font-size: 26px;")
        self.boton_contrasena.clicked.connect(self.establecer_contrasena)
        self.layout.addWidget(self.boton_contrasena)

        self.setLayout(self.layout)

    def establecer_contrasena(self):
        nueva = self.input.text()
        if nueva:
            self.senal_nueva_contrasena.emit(nueva)
            self.hide()
            self.senal_ir_entrada.emit()
        else:
            self.label_input.setText("La contraseña no puede estar vacía.")
...
class Entrada(QWidget):
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if x in range(395, 620) and y in range(560, 905):
            self.label_contrasena.show()
            self.input_contrasena.show()
        else:
            self.label_contrasena.hide()
            self.input_contrasena.hide()
    ...
    def keyPressEvent(self, event: QKeyEvent) -> None:
        if event.key() == 16777220:  # Enter
            input = self.input_contrasena.text()
            if input:
                self.senal_emitir_contrasena.emit(input)
            else:
                self.label_contrasena.setText(
                    "La contraseña no puede estar vacía. Intenta de nuevo.")
