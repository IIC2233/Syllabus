...
class Frontend(QObject):
    ...
    senal_validar_clave = pyqtSignal(str)
    ...
    def conectar_senales(self) -> None:
        ...
        self.ventana_inicio.senal_enviar_clave.connect(
            self.senal_validar_clave.emit)

        self.ventana_perfiles.senal_volver_inicio.connect(self.volver_a_inicio)
        self.ventana_perfiles.senal_abrir_perfil.connect(self.abrir_perfil)

        for nombre_usuario, perfil in self.ventana_perfil.items():
            perfil.senal_volver_perfiles.connect(self.volver_a_perfiles)
