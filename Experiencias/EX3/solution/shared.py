...
def send_message(sock: socket.socket, payload: bytes) -> None:
    ...
    """
    largo = len(payload)
    header = largo.to_bytes(4, byteorder="big")
...
def recv_exactly(sock: socket.socket, n: int) -> bytes:
    ...
    while len(buffer) < n:
        ...
        if not chunk:  # recv vacío = conexión cerrada
            raise ConnectionError("Peer cerró la conexión antes de completar")
        buffer.extend(chunk)
    return bytes(buffer)
...
def recv_message(sock: socket.socket) -> bytes:
    ...
    """
    len = recv_exactly(sock, 4)
    largo = int.from_bytes(len, byteorder="big")
    payload = recv_exactly(sock, largo)
    return payload
