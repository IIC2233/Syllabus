...
class Juego(QObject):
    senal_a_frontend = pyqtSignal(dict)
    # senal_mover_meteorito = pyqtSignal(int, int, int)
    # senal_remover_meteorito = pyqtSignal(int)
    senal_fin_meteorito = pyqtSignal(int, bool)  # meteorito->backent
    ...
    def aparecer_meteorito(self, post_x: int) -> None:
        ...
        meteorito = Meteorito(
            ...
            senal_a_frontend=self.senal_a_frontend
        )
        ...
        self.senal_a_frontend.emit(
            {
                'tipo':'aparecer_meteorito',
                'args': (meteorito.id, meteorito.x, meteorito.y)
            }
        )
        # self.senal_aparecer_meteorito.emit(meteorito.id, meteorito.x, meteorito.y)

    def actualizar_poblacion(self, poblacion: int) -> None:
        texto = "DCCiudad ha perdido todos sus ciudadanos"
        if poblacion > 0:
            texto = f"DCCiudad tiene {poblacion} ciudadanos"
        
        self.senal_a_frontend.emit({
            'tipo': 'actualizar_poblacion',
            'args': (texto, )
        })
        # if poblacion <= 0:
